...
class _ManageAkademikPageState extends State<ManageAkademikPage> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadAllData() async {
    ...
    try {
      ...
      final futures = await Future.wait([
        ...
        client.from('mata_kuliah').select('*, program_studi(nama), semester(nama)').order('kode'),
        client.from('dosen').select('*, users(nama)'),
        client.from('kelas').select('*, mata_kuliah(nama, kode), dosen(users(nama))').order('nama'),
        client.from('jadwal').select('*, kelas(*, mata_kuliah(*), dosen(users(nama))), semester(*)').order('hari'),
      ]);
  ...
  Future<void> _addOrEditProdi([Map<String, dynamic>? item]) async {
    ...
    if (ok == true) {
      if (kodeController.text.trim().isEmpty || namaController.text.trim().isEmpty) return;
      setState(() => _isLoading = true);
      try {
        final client = SupabaseConfig.client;
        if (item == null) {
          ...
          await client.from('program_studi').update({
            'kode': kodeController.text.trim().toUpperCase(),
            'nama': namaController.text.trim(),
          }).eq('id', item['id']);
        }
  ...
  Future<void> _deleteProdi(String id) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: const Text("Yakin ingin menghapus prodi ini? Semua data terkait (mahasiswa, matkul, dll) akan terhapus."),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context, false), child: const Text("Batal")),
          ElevatedButton(
  ...
  Future<void> _addOrEditSemester([Map<String, dynamic>? item]) async {
    ...
    if (ok == true) {
      ...
      try {
        ...
        if (item == null) {
          final newSem = await client.from('semester').insert({
            'nama': namaController.text.trim(),
            'status': status,
          }).select().single();
          
          if (status == true) {
            await client.from('semester').update({'status': false}).neq('id', newSem['id']);
          }
        } else {
          await client.from('semester').update({
            'nama': namaController.text.trim(),
            'status': status,
          }).eq('id', semId!);

          if (status == true) {
            await client.from('semester').update({'status': false}).neq('id', semId);
          }
        }
  ...
  Future<void> _addOrEditMatkul([Map<String, dynamic>? item]) async {
    ...
    final namaController = TextEditingController(text: item?['nama'] ?? '');
    int sks = item?['sks'] ?? 2;
    String? prodiId = item?['program_studi_id'] ?? (_prodiList.isNotEmpty ? _prodiList.first['id'] : null);
    String? semesterId = item?['semester_id'] ?? (_semesterList.isNotEmpty ? _semesterList.first['id'] : null);
    ...
    if (ok == true) {
      if (kodeController.text.trim().isEmpty || namaController.text.trim().isEmpty || prodiId == null || semesterId == null) return;
      setState(() => _isLoading = true);
      try {
        final client = SupabaseConfig.client;
        final data = {
  ...
  Future<void> _addOrEditKelas([Map<String, dynamic>? item]) async {
    final namaController = TextEditingController(text: item?['nama'] ?? '');
    final kuotaController = TextEditingController(text: item?['kuota']?.toString() ?? '40');
    String? mkId = item?['mata_kuliah_id'] ?? (_matkulList.isNotEmpty ? _matkulList.first['id'] : null);
    String? dosenId = item?['dosen_id'] ?? (_dosenList.isNotEmpty ? _dosenList.first['id'] : null);
    ...
    if (ok == true) {
      ...
      if (kName.isEmpty || kuota == null || mkId == null || dosenId == null) return;
  ...
  Future<void> _addOrEditJadwal([Map<String, dynamic>? item]) async {
    String? kelasId = item?['kelas_id'] ?? (_kelasList.isNotEmpty ? _kelasList.first['id'] : null);
    String hari = item?['hari'] ?? 'Senin';
    String ruangan = item?['ruangan'] ?? 'Ruang 101';
    String? semId = item?['semester_id'] ?? (_semesterList.isNotEmpty ? _semesterList.first['id'] : null);
                  ...
                  DropdownButtonFormField<String>(
                    ...
                    items: _kelasList.map((k) {
                      ...
                      return DropdownMenuItem(
                        value: k['id'].toString(),
                        child: Text("${k['nama']} - ${mk['nama']} ($d)", overflow: TextOverflow.ellipsis),
                      );
                    }).toList(),
  ...
  Future<void> _deleteItem(String table, String id) async {
    final ok = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: const Text("Yakin ingin menghapus item ini? Tindakan ini akan menghapus data yang bergantung."),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context, false), child: const Text("Batal")),
          ElevatedButton(
            style: ElevatedButton.styleFrom(backgroundColor: AppTheme.accent, foregroundColor: Colors.white),
  ...
  Widget _buildProdiList() {
    if (_prodiList.isEmpty) return const Center(child: Text("Belum ada data Prodi"));
        ...
        return Card(
          child: ListTile(
            ...
            trailing: Row(
              mainAxisSize: MainAxisSize.min,
              children: [
                IconButton(icon: const Icon(Icons.edit, color: Colors.blue), onPressed: () => _addOrEditProdi(item)),
                IconButton(icon: const Icon(Icons.delete, color: AppTheme.accent), onPressed: () => _deleteProdi(item['id'])),
              ],
            ),
          ),
        );
  ...
  Widget _buildSemesterList() {
    if (_semesterList.isEmpty) return const Center(child: Text("Belum ada data Semester"));
    return ListView.builder(
      padding: const EdgeInsets.all(16),
        ...
        return Card(
          child: ListTile(
            ...
            trailing: Row(
              mainAxisSize: MainAxisSize.min,
              children: [
                IconButton(icon: const Icon(Icons.edit, color: Colors.blue), onPressed: () => _addOrEditSemester(item)),
                IconButton(icon: const Icon(Icons.delete, color: AppTheme.accent), onPressed: () => _deleteItem('semester', item['id'])),
              ],
            ),
          ),
        );
