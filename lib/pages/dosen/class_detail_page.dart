...
class _ClassDetailPageState extends State<ClassDetailPage> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadAllClassData() async {
    ...
    try {
      ...
      final futures = await Future.wait([
        ...
        jId != null 
            ? client.from('krs').select('*, mahasiswa(*, users(nama))').eq('jadwal_id', jId)
            : Future.value([]),
        // 2. Materials
        client.from('materi').select().eq('kelas_id', kId).order('created_at', ascending: false),
        // 3. Assignments
        client.from('tugas').select().eq('kelas_id', kId).order('created_at', ascending: false),
        // 4. Student Grades
        client.from('nilai').select('*, mahasiswa(*, users(nama))').eq('kelas_id', kId),
        // 5. Announcements for this class
        client.from('pengumuman').select('*, users(nama)').eq('kelas_id', kId).order('created_at', ascending: false),
      ]);
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text("Error memuat detail kelas: $e"), backgroundColor: Colors.red),
        );
      }
    }
  }
            ...
            content: Column(
              ...
              children: [
                ...
                DropdownButtonFormField<String>(
                  value: tipe,
                  decoration: const InputDecoration(labelText: "Tipe File/Sumber"),
                  items: const [
                    DropdownMenuItem(value: 'pdf', child: Text("PDF Document")),
                    DropdownMenuItem(value: 'word', child: Text("Word Document")),
                    DropdownMenuItem(value: 'ppt', child: Text("PowerPoint")),
                    DropdownMenuItem(value: 'video', child: Text("Video")),
                    DropdownMenuItem(value: 'gambar', child: Text("Gambar")),
                    DropdownMenuItem(value: 'link', child: Text("Tautan Eksternal (Drive/YouTube)")),
                  ],
                  onChanged: (val) {
                ...
                if (!isLink) ...[
                  ...
                  TextField(
                    decoration: const InputDecoration(labelText: "URL Tautan (e.g. YouTube/Drive)"),
    ...
    if (result == true) {
      ...
      try {
        ...
        if (tipe != 'link' && pickerResult != null) {
          ...
          final storagePath = "materi/$fileName";

          if (kIsWeb) {
            if (file.bytes != null) {
              await client.storage.from('materi').uploadBinary(storagePath, file.bytes!);
            } else {
              throw Exception("Web file bytes empty");
            }
          } else {
            if (file.path != null) {
              await client.storage.from('materi').upload(storagePath, File(file.path!));
            } else {
    ...
    if (ok == true) {
      ...
      try {
        ...
        if (url.contains('/storage/v1/object/public/materi/')) {
          ...
          await client.storage.from('materi').remove(["materi/$pathSegment"]);
        }
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 12),
                  TextField(controller: descController, maxLines: 3, decoration: const InputDecoration(labelText: "Deskripsi Tugas")),
                  const SizedBox(height: 16),
                  ...
                  Row(
                    children: [
                      ...
                      Expanded(
                        child: Text(
                          attachmentName.isEmpty ? "Tidak ada lampiran" : attachmentName,
                          maxLines: 1,
                          overflow: TextOverflow.ellipsis,
                          style: const TextStyle(fontSize: 12, color: AppTheme.textLight),
                        ),
    ...
    if (result == true) {
      ...
      try {
        ...
        if (pickerResult != null) {
          final file = pickerResult!.files.first;
          final fileName = "${DateTime.now().millisecondsSinceEpoch}_${file.name}";
          final storagePath = "tugas/$fileName";

          if (kIsWeb) {
            if (file.bytes != null) {
              await client.storage.from('tugas').uploadBinary(storagePath, file.bytes!);
            }
          } else {
            if (file.path != null) {
              await client.storage.from('tugas').upload(storagePath, File(file.path!));
            }
          }
  ...
  Future<void> _deleteTugas(String id, String? url) async {
    final ok = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: const Text("Yakin ingin menghapus tugas ini beserta seluruh pengumpulan mahasiswa?"),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context, false), child: const Text("Batal")),
          ElevatedButton(
            style: ElevatedButton.styleFrom(backgroundColor: AppTheme.accent, foregroundColor: Colors.white),
    ...
    if (ok == true) {
      ...
      try {
        ...
        if (url != null && url.contains('/storage/v1/object/public/tugas/')) {
          final uri = Uri.parse(url);
          final pathSegment = uri.pathSegments.last;
          await client.storage.from('tugas').remove(["tugas/$pathSegment"]);
        }
  ...
  Future<void> _editNilai(Map<String, dynamic> gradeRow) async {
    final tController = TextEditingController(text: gradeRow['nilai_tugas']?.toString() ?? '');
    final uController = TextEditingController(text: gradeRow['nilai_uts']?.toString() ?? '');
    final aController = TextEditingController(text: gradeRow['nilai_uas']?.toString() ?? '');
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            TextField(controller: tController, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: "Nilai Tugas (30%)")),
            const SizedBox(height: 12),
            TextField(controller: uController, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: "Nilai UTS (30%)")),
            const SizedBox(height: 12),
            TextField(controller: aController, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: "Nilai UAS (40%)")),
          ],
        ),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context, false), child: const Text("Batal")),
    ...
    if (ok == true) {
      ...
      if (tScore < 0 || tScore > 100 || uScore < 0 || uScore > 100 || aScore < 0 || aScore > 100) {
        _snack("Nilai harus berada di rentang 0 - 100", Colors.orange);
        return;
      }

      // Compute Final Score (Nilai Akhir)
      final double finalScore = (tScore * 0.3) + (uScore * 0.3) + (aScore * 0.4);
      ...
      setState(() => _isLoading = true);
      try {
        await SupabaseConfig.client.from('nilai').update({
          'nilai_tugas': tScore,
          'nilai_uts': uScore,
          'nilai_uas': aScore,
      ...
      builder: (_) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            TextField(controller: contentController, maxLines: 3, decoration: const InputDecoration(labelText: "Isi")),
          ],
        ),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context, false), child: const Text("Batal")),
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Card(
                      child: ListTile(
                        ...
                        subtitle: Text("Tipe: ${item['tipe_file'].toUpperCase()}"),
                        trailing: IconButton(
                          icon: const Icon(Icons.delete, color: AppTheme.accent),
                          onPressed: () => _deleteMateri(item['id'].toString(), item['url_file'].toString()),
                        ),
                      ),
                    );
                  },
          ...
          child: _tugasList.isEmpty
              ...
              : ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final dlStr = "${dl.day}/${dl.month}/${dl.year} ${dl.hour.toString().padLeft(2, '0')}:${dl.minute.toString().padLeft(2, '0')}";
                      ...
                      child: ListTile(
                        ...
                        trailing: Row(
                          ...
                          children: [
                            ...
                            IconButton(
                              icon: const Icon(Icons.delete, color: AppTheme.accent),
                              onPressed: () => _deleteTugas(item['id'].toString(), item['url_file']?.toString()),
                            ),
                          ],
                        ),
                      ),
  ...
  Widget _buildGradesTab() {
    if (_gradesList.isEmpty) {
      return const Center(child: Text("Belum ada mahasiswa terdaftar (KRS harus disetujui dahulu)"));
    }
                ...
                Row(
                  ...
                  children: [
                    ...
                    ElevatedButton(
                      ...
                      child: const Text("Input Nilai", style: TextStyle(fontSize: 12)),
                    )
                  ],
                ),
                Text("NIM: $nim | Grade: $grade", style: const TextStyle(fontSize: 12, color: AppTheme.textLight)),
  ...
  Widget _buildScoreVal(String label, String score) {
    return Column(
      children: [
        ...
        Text(score, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: AppTheme.textDark)),
      ],
    );
  }
      ...
      children: [
        ...
        Expanded(
          child: _announcements.isEmpty
              ? const Center(child: Text("Belum ada pengumuman untuk kelas ini"))
              : ListView.builder(
                  padding: const EdgeInsets.symmetric(horizontal: 16),
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Card(
                      child: ListTile(
                        title: Text(ann['judul'] ?? '', style: const TextStyle(fontWeight: FontWeight.bold)),
                        subtitle: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            ...
                            Text(ann['isi'] ?? ''),
                            const SizedBox(height: 4),
                            Text("Tanggal: $dateStr", style: const TextStyle(fontSize: 11, color: AppTheme.textLight)),
                          ],
                        ),
                        trailing: IconButton(
                          icon: const Icon(Icons.delete, color: AppTheme.accent),
