...
class _MhsClassDetailPageState extends State<MhsClassDetailPage> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadClassContent() async {
    ...
    try {
      ...
      final futures = await Future.wait([
        ...
        client.from('nilai').select().eq('kelas_id', kId).eq('mahasiswa_id', mId).maybeSingle(),
      ]);

      _materiList = List<Map<String, dynamic>>.from(futures[0] as List);
      _tugasList = List<Map<String, dynamic>>.from(futures[1] as List);
      
      final submissionsList = List<Map<String, dynamic>>.from(futures[2] as List);
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text("Gagal memuat materi/tugas: $e"), backgroundColor: Colors.red),
        );
      }
    }
  }
  ...
  Future<void> _submitTugas(String tugasId, String tugasTitle, Map<String, dynamic>? existingSub) async {
    ...
    if (existingSub != null) {
      final confirm = await showDialog<bool>(
        ...
        builder: (_) => AlertDialog(
          ...
          content: const Text("Anda sudah mengumpulkan jawaban. Apakah Anda yakin ingin menggantinya dengan file baru?"),
          actions: [
            TextButton(onPressed: () => Navigator.pop(context, false), child: const Text("Batal")),
            ElevatedButton(onPressed: () => Navigator.pop(context, true), child: const Text("Ganti")),
          ],
        ),
      );
    ...
    try {
      ...
      if (existingSub == null) {
        ...
        await client.from('pengumpulan_tugas').update({
          'file_jawaban': fileUrl,
          'waktu_kumpul': DateTime.now().toIso8601String(),
              ...
              children: [
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: Text(
                        ...
                        style: TextStyle(color: isSubmitted ? Colors.green : Colors.orange, fontSize: 10, fontWeight: FontWeight.bold),
                      ),
                    ),
                  ],
                ),
                const SizedBox(height: 6),
                Text("Deadline: $dlStr", style: TextStyle(color: isPassed ? Colors.red : AppTheme.primary, fontSize: 12, fontWeight: FontWeight.bold)),
                ...
                if (hasAttachment)
                  TextButton.icon(
                    ...
                    label: const Text("Unduh Lampiran Soal", style: TextStyle(fontSize: 12)),
                ...
                if (isSubmitted && mySub['nilai'] != null) ...[
                  ...
                  Row(
                    children: [
                      const Text("Nilai: ", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                      Text(
                ...
                if (!isPassed) ...[
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton.icon(
                      ...
                      label: Text(isSubmitted ? "UBAH JAWABAN" : "KUMPULKAN JAWABAN"),
                      onPressed: () => _submitTugas(tId, title, mySub),
                    ),
                  ),
                ] else if (!isSubmitted) ...[
                  const SizedBox(height: 12),
                  const Text("Tenggat waktu pengumpulan telah lewat.", style: TextStyle(color: Colors.red, fontSize: 12, fontWeight: FontWeight.bold)),
                ]
              ],
            ),
          ),
            ...
            children: [
              Card(
                ...
                child: Padding(
                  ...
                  child: Column(
                    children: [
                      const Text("NILAI AKHIR KULIAH", style: TextStyle(color: AppTheme.textLight, fontSize: 13, fontWeight: FontWeight.bold, letterSpacing: 0.5)),
                      ...
                      const SizedBox(height: 16),
                      Text("Skor Akhir: $akhir", style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: AppTheme.textDark)),
  ...
  Widget _buildGradeRow(String label, String score) {
    return Row(
      ...
      children: [
        ...
        Text(score, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14, color: AppTheme.textDark)),
      ],
    );
  }
}
